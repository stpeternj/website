 ...
 * @since 1.0
 */
function saved_add_theme_support_ctc() {
	...
	add_theme_support( 'ctc-people', array(
		...
		'field_overrides' => array(
			'_ctc_person_urls' => array(
				'desc' => sprintf( __( 'Enter one URL per line for %s.', 'saved' ), saved_social_icon_sites( 'or' ) ),
			),
			'_ctc_person_email' => array(
				'desc' => __( 'Email address will be encoded to help prevent spam.', 'saved' ),
			),
		),
	) );
...
add_action( 'after_setup_theme', 'saved_add_theme_support_ctc' );
