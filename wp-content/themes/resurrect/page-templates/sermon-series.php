...
function resurrect_sermon_series_after_content() {
	...
	<div id="resurrect-sermon-series" class="resurrect-sermon-index<?php if ( ctfw_has_content() ) : ?> resurrect-sermon-index-has-content<?php endif; ?>">
		...
		<?php if ( $series_by_year ) : ?>
			...
			<div id="resurrect-sermon-series-list" class="resurrect-clearfix">

				<?php foreach ( $series_by_year as $year => $series ) : ?>

					<h2><?php echo $year; ?></h2>

					<ul class="resurrect-clearfix">
						...
						<?php foreach ( $series as $series_data ) : ?>
							...
							<li>
								...
								<div class="resurrect-sermon-series-dates">
									...
									if ( $series_data->sermon_earliest_date != $series_data->sermon_latest_date ) :
										...
										printf(
											...
											_x( '%s &ndash; %s', 'series date range', 'resurrect' ),
								...
								<div class="resurrect-sermon-series-count">
									...
									if ( 1 === $series_data->count ) {
										...
										printf(
											...
											_x( '%1$s %2$s', 'sermon count single', 'resurrect' ),
											$series_data->count,
									...
									// Multiple sermons.
										...
										printf(
											...
											_x( '%1$s %2$s', 'sermon count multiple', 'resurrect' ),
											$series_data->count,
											$sermon_word_plural
										);
		...
		<?php else : ?>
			...
			<p id="resurrect-sermon-index-none">
				<?php _e( 'There are no series to show.', 'resurrect' ); ?>
			</p>
...
add_action( 'resurrect_after_content', 'resurrect_sermon_series_after_content' );
