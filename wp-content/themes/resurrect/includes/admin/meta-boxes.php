 ...
 * @since 1.0
 */
function resurrect_add_meta_box_options() {
	...
	foreach ( $post_types as $post_type ) {
		...
		$meta_box = array(
			...
			// Meta Box
			'id' 		=> 'resurrect_options', // unique ID
			'title' 	=> __( 'Options', 'resurrect' ),
			'post_type'	=> $post_type,
		...
		if ( 'page' == $post_type ) {
			...
			$meta_box['fields']['_ctcom_banner_no_text'] = array( // usiing _ctcom_ prefix for portability between themes
				'name'				=> __( 'Banner', 'resurrect' ),
				'after_name'		=> '', // (Optional), (Required), etc.
				'desc'				=> '',
				'type'				=> 'checkbox', // text, textarea, checkbox, checkbox_multiple, radio, select, number, upload, upload_textarea, url
				'checkbox_label'	=> __( 'Do not show text over featured image', 'resurrect' ), //show text after checkbox
			...
			$meta_box['fields']['_ctcom_banner_related'] = array( // usiing _ctcom_ prefix for portability between themes
				'name'				=> '',
				'after_name'		=> '', // (Optional), (Required), etc.
				'desc'				=> __( "Example: Sermons template used. This page's banner shows on all sermon pages.", 'resurrect' ),
				'type'				=> 'checkbox', // text, textarea, checkbox, checkbox_multiple, radio, select, number, upload, upload_textarea, url
				'checkbox_label'	=> __( "Show banner on related content", 'resurrect' ), //show text after checkbox
				'options'			=> array(), // array of keys/values for radio or select
				'upload_button'		=> '', // text for button that opens media frame
				'upload_title'		=> '', // title appearing at top of media frame
				'upload_type'		=> '', // optional type of media to filter by (image, audio, video, application/pdf)
				...
				'custom_sanitize'	=> 'resurrect_sanitize_banner_related', // function to do additional sanitization
				'custom_field'		=> '', // function for custom display of field input
		...
		$meta_box['fields']['_ctcom_hide_sidebar'] = array( // usiing _ctcom_ prefix for portability between themes
			'name'				=> _x( 'Sidebar', 'meta box', 'resurrect' ),
			'after_name'		=> '', // (Optional), (Required), etc.
			'desc'				=> '',
			'type'				=> 'checkbox', // text, textarea, checkbox, checkbox_multiple, radio, select, number, upload, upload_textarea, url
			'checkbox_label'	=> sprintf( __( 'Hide sidebar <a href="%s" target="_blank">widgets</a>', 'resurrect' ), admin_url( 'widgets.php' ) ), //show text after checkbox
			'options'			=> array(), // array of keys/values for radio or select
			'upload_button'		=> '', // text for button that opens media frame
			'upload_title'		=> '', // title appearing at top of media frame
			'upload_type'		=> '', // optional type of media to filter by (image, audio, video, application/pdf)
		...
		$meta_box['fields']['_ctcom_gallery_exclude'] = array( // usiing _ctcom_ prefix for portability between themes
			'name'				=> _x( 'Gallery', 'meta box', 'resurrect' ),
			'after_name'		=> '', // (Optional), (Required), etc.
			'desc'				=> __( 'Applies only when gallery used in content.', 'resurrect' ),
			'type'				=> 'checkbox', // text, textarea, checkbox, checkbox_multiple, radio, select, number, upload, upload_textarea, url
			'checkbox_label'	=> __( 'Exclude from gallery widgets/templates', 'resurrect' ), //show text after checkbox
			'options'			=> array(), // array of keys/values for radio or select
			'upload_button'		=> '', // text for button that opens media frame
			'upload_title'		=> '', // title appearing at top of media frame
			'upload_type'		=> '', // optional type of media to filter by (image, audio, video, application/pdf)
...
add_action( 'admin_init', 'resurrect_add_meta_box_options' );
...
function resurrect_sanitize_banner_related( $value ) {
		...
		if ( ! empty( $page_template ) && 'default' != $page_template ) {
			...
			if ( $content_type ) {
				...
				if ( ! empty( $content_type_page_templates ) ) {
					...
					$banner_pages = resurrect_banner_related_pages();
