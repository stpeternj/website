 ...
 *
 * For social icons to filter, see resurrect_social_icon_map() below.
 *
 * @since 1.0
 * @param string $element Element icon used with
 * @return string Icon class
 */
function resurrect_get_icon_class( $element ) {
	...
	$classes = array(
		...
		'event-excluded'		=> 'el-icon-remove-circle',
	...
	$classes = apply_filters( 'resurrect_icon_classes', $classes, $element );
	...
	// Return filterable
	return apply_filters( 'resurrect_get_icon_class', $class, $element );
 ...
 * Output icon class
 *
 * Output contents of resurrect_get_icon_class()
 *
 * @since 1.0
 * @param string $element Element icon used with
 ...
 */
function resurrect_icon_class( $element, $return = false ) {

	$class = apply_filters( 'resurrect_icon_class', resurrect_get_icon_class( $element ) );
 ...
 * Icons available
 *
 * This is used in displaying icons with resurrect_social_icons() and
 * to tell which social networks are supported with resurrect_social_icon_sites().
 *
 * @since 1.0
 * @return array Icon map
 */
function resurrect_social_icon_map() {
	...
	$icon_map = array(
		...
		'el-icon-dribble'		=> array( 	'dribbble',			'Dribbble',			true ),
		'el-icon-envelope'		=> array( 	array( 'mailto', 'newsletter' ), 'Email' ),
		'el-icon-podcast'		=> array( 	array( 'itunes', 'podcast', 'sermonaudio.com', 'play.google.com', 'playmusic.app.goo.gl', 'castbox', 'libsyn' ), 'Podcast' ),
	...
	// Return filtered
	return apply_filters( 'resurrect_social_icon_map', $icon_map );
 ...
 */
function resurrect_social_icon_sites( $or = false ) {

	$icon_map = resurrect_social_icon_map();
	...
	foreach ( $icon_map as $site_data ) { // make list of sites with icons
		...
		if ( $i > 1 ) { // not first one
			if ( $i < $sites_with_icons_count ) { // not last one
				$sites_with_icons .= _x( ', ', 'social icons list', 'resurrect' );
			} else { // last one
				if ( ! empty( $or ) ) {
					$sites_with_icons .= _x( ' or ', 'social icons list', 'resurrect' );
				} else {
					$sites_with_icons .= _x( ' and ', 'social icons list', 'resurrect' );
				}
			}
		}
	...
	return apply_filters( 'resurrect_social_icon_sites', $sites_with_icons );
 ...
 */
function resurrect_social_icons( $urls, $return = false ) {
	...
	if ( ! empty( $urls ) ) {
				...
				foreach ( $icon_map as $icon_class => $site_data ) {
					...
					foreach ( $url_checks as $url_match ) {
						...
						if ( preg_match( '/' . preg_quote( $url_match ) . '/i', $url ) && ! $url_matched ) {
							...
							// Append icon
							$icon_items .= '	<li><a href="' . esc_url( $url, $protocols ) . '" class="' . esc_attr( $icon_class ) . '" title="' . esc_attr( $name ) . '" target="' . apply_filters( 'resurrect_social_icons_link_target', '_blank' ) . '"></a></li>' . "\n";
		...
		if ( ! empty( $icon_items ) ) {
			$icon_list = '<ul class="resurrect-list-icons">' . "\n";
	...
	$icon_list = apply_filters( 'resurrect_social_icons', $icon_list, $urls );
	if ( $return ) {
		return $icon_list;
	} else {
		echo $icon_list;
