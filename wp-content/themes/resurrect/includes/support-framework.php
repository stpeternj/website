 ...
 * @since 1.0
 */
function resurrect_add_theme_support_framework() {
	...
	add_theme_support( 'ctfw-wordpress-version', '5.0' ); // 5.0 for block starter content, 4.7 for get_theme_file_uri(), 3.6 at first release.
	...
	add_theme_support( 'ctfw-after-activation', array(
		...
		'notice'				=> sprintf( __( '<b>Next Steps:</b> Please continue reading the <a href="%s" target="_blank">Getting Started</a> guide for the next steps after theme activation.', 'resurrect' ), 'https://churchthemes.com/guides/user/getting-started/' ),
	...
	add_theme_support( 'ctfw-edd-license', array(
		...
		'options_page_message'		=> sprintf( // shown at top of Theme License page
											__( 'Please activate your license key to enable one-click theme updates and to get support from <a href="%1$s" target="_blank">ChurchThemes.com</a> for this site. Read <a href="%2$s" target="_blank">License Keys</a> for more information.', 'resurrect' ),
											'https://churchthemes.com',
											'https://churchthemes.com/go/license-keys'
										),
		'activation_error_notice'	=> sprintf( // shown when activation fails
											__( '<b>License key could not be activated.</b> Read the <a href="%1$s" target="_blank">License Keys</a> guide for help.', 'resurrect' ),
											'https://churchthemes.com/go/license-inactive'
										),
		'expired_notice'			=> __( '<strong>Theme License Expired:</strong> <a href="%4$s" target="_blank">Renew</a> your <a href="%1$s">Theme License</a> to re-enable updates and support for the <strong>%2$s</strong> theme (expired on <strong>%3$s</strong>). <a href="%5$s" target="_blank">Learn More</a>', 'resurrect' ), // optional notice to override default with when license is expired
		'expiring_soon_notice'		=> __( '<strong>Theme License Expiring Soon:</strong> <a href="%4$s" target="_blank">Renew</a> your <a href="%1$s">Theme License</a> to continue receiving updates and support for the <strong>%2$s</strong> theme (expires on <strong>%3$s</strong>). <a href="%5$s" target="_blank">Learn More</a>', 'resurrect' ), // optional notice to override default with when license expires soon
	...
	// WordPress core and its translations sometimes use text that is not preferred (e.g. too long)
 	add_theme_support( 'ctfw-replace-wp-text', array(
        'Correo electrónico'	=> _x( 'Email', 'contact form', 'resurrect' ), // Spanish: too long for comment form
        'Adresse de contact'	=> _x( 'Courriel', 'contact form', 'resurrect' ), // French: too long for comment form
    ) );
	...
	add_theme_support( 'ctfw-preset-backgrounds', array(
		'asphalt.jpg' => array(
			...
			'preset' 		=> 'fill',					// WordPress core "Presets" <select>: default, fill, fit, repeat, custom
		...
		'grunge.png' => array(
			...
			'preset' 		=> 'repeat',				// WordPress core "Presets" <select>: default, fill, fit, repeat, custom
			'colorable' 	=> true
		),
		'screen.png' => array(
	...
	// Page templates must be specified with ctfw_content_types filter. See resurrect_update_content_types().
	...
	// This includes months in past, beyond next year, invalid format or invalid category
	...
	add_theme_support( 'ctfw-loop-after-content-used', 'resurrect_loop_after_content_used' );
	...
	add_theme_support(
		...
		array(
			...
			'page'			=> 'resurrect-banner'
		),
		__( 'Optionally provide an image that is at least %s (it will be cropped/resized). An image exactly this size is ideal.', 'resurrect' )
	);
	...
	add_theme_support( 'ctfw-gallery-thumb-size', array(
		'3' => 'resurrect-rect-large', 		// when 1 to 3 columns used
		'5' => 'resurrect-rect-medium', 	// when 4 to 5 columns used
		'9' => 'resurrect-rect-small',  	// when 6 to 9 columns used
	) );
	...
	add_theme_support( 'ctfw-editor-styles', array(
		...
		'css_function'			=> 'resurrect_head_styles',								// For Classic Editor. Function outputting dynamic CSS in <head> (exclude <style> tag).
		'block_css_function'	=> 'resurrect_block_editor_head_styles',				// For Gutenberg Editor. Function outputting dynamic CSS in admin <head> (exclude <style> tag).
	...
	add_theme_support( 'ctfw-import-correct-urls', array(
		...
		'multisite_id'	=> 11, // site ID if imported files are coming off of a multisite installation
	) );
...
add_action( 'after_setup_theme', 'resurrect_add_theme_support_framework' );
 ...
 * @since 1.0
 */
function resurrect_add_theme_support_framework_widgets() {
	...
	add_theme_support( 'ctfw-widget-archives', array(
		...
		// See resurrect_archive_widget_types() below for specifying which post types this theme supports archives for.
	...
	add_theme_support( 'ctfw-widget-slide', array(
		...
		'field_overrides' => array(
			'image_id' => array( // tell user image is required with this theme
				'after_name' => _x( '(Required)', 'slide widget image', 'resurrect' ),
				'desc' => sprintf( __( 'Image cropped to %s.', 'resurrect' ), ctfw_image_size_dimensions( 'resurrect-slide' ) ),
			),
			'video' => array( // tell user which video URLs to use
				'desc' => __( 'To make this a video slide, enter a YouTube or Vimeo video page URL.', 'resurrect' ),
			),
		),
	) );
	...
	add_theme_support( 'ctfw-widget-highlight', array(
		...
		'field_overrides' => array(
			'image_id' => array(
				'desc' => sprintf( __( 'Image cropped to %s.', 'resurrect' ), ctfw_image_size_dimensions( 'resurrect-rect-large' ) ),
			),
		),
	) );
...
add_action( 'after_setup_theme', 'resurrect_add_theme_support_framework_widgets' );
 ...
 */
function resurrect_archive_widget_types( $options ) {
	...
	return apply_filters( 'resurrect_archive_widget_types', $options );

}

add_filter( 'ctfw_archives_widget_post_type_options', 'resurrect_archive_widget_types' );
