 ...
 * @since 1.0
 */
function resurrect_add_theme_support_ctc() {
	...
	add_theme_support( 'ctc-people', array(
		...
		'field_overrides' => array(
			'_ctc_person_urls' => array(
				'desc' => sprintf( __( 'Enter one URL per line for %s.', 'resurrect' ), resurrect_social_icon_sites( 'or' ) ),
			),
			'_ctc_person_email' => array(
				'desc' => __( 'Email address will be encoded to help prevent spam.', 'resurrect' ),
			),
		),
	) );
...
add_action( 'after_setup_theme', 'resurrect_add_theme_support_ctc' );
